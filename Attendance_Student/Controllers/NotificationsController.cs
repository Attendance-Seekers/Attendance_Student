...
namespace Attendance_Student.Controllers
{
    ...
    public class NotificationsController : ControllerBase
    {
        ...
        [HttpGet]
        [SwaggerOperation(Summary = "Receive notifications for a parent with pagination", Description = "Retrieves paginated notifications for a specific parent and includes the attendance status of their children.")]
        [SwaggerResponse(StatusCodes.Status200OK, "Notifications received successfully.", Type = typeof(PaginatedResponse<SelectNotificationDTO>))]
        [SwaggerResponse(StatusCodes.Status400BadRequest, "No notifications found for the given parent ID.")]
        [Produces("application/json")]
        public async Task<IActionResult> ReceiveNotifications(string parent_id, [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
        {
            ...
            foreach (var notification in paginatedNotifications)
            {
                List<SudentNotificationDTO> studentDTOs = new List<SudentNotificationDTO>();
            ...
            var response = new PaginatedResponse<SelectNotificationDTO>
            {
                TotalCount = totalCount,
                TotalPages = totalPages,
                CurrentPage = page,
                PageSize = pageSize,
                Data = selectNotifications
            };
